...
### Deploy with CI
Push your changes to a merge request and run the GitLab pipelines `add_models:dev` and `add_models:prod`.
...
When running locally, define a `.env` file in the `custom/scripts` directory with required environment variables. You can grab the secret values from GitLab CI, e.g.:
