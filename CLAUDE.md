...
- Adding at least 1 test is required for all PRs
- Tests should be added to `tests/test_litellm/` directory (mirrors `litellm/` structure)
- Use mocked responses in unit tests, not real API calls
- Sign the Contributor License Agreement (CLA) before contributing

### Code Quality Standards
- All code must pass `make lint` before merge
- Follow existing patterns in provider implementations
- Use appropriate base classes from `litellm/llms/base.py`
- Maintain backwards compatibility for public APIs

### Working with Providers
- Provider-specific code goes in `litellm/llms/{provider_name}/`
- Each provider needs chat, completion, and embedding transformations
- Cost calculation logic in provider's `cost_calculation.py`
- Common utilities in provider's `common_utils.py`